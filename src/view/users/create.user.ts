...
import { AccountTabedata, CreateUserModel, UserData } from "../../model/User.model.js";
...
router.post("/", (req, res) => {
    requireUser(req, res, async ({ id, dbCode, accountCode }: { id: number, dbCode: string, accountCode: string }) => {
        try {
            const data = req.body
            ...
            const accountTabedata: AccountTabedata = {
                email: data.email,
