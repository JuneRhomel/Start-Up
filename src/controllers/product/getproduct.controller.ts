 ...
 */
export function getProductHandler(dbCode: string, res: Response): Response {
 ...
 */
export function getProductHandlerById(dbCode: string, id: number, res: Response): Response {
    return new Promise(async (resolve, reject) => {
        const sql = `
        SELECT product.id, product.product_name, product.price, status_list.status_name as status 
        FROM ${dbCode}.product
